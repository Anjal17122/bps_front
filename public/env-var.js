window.globalConfig = {
  name: "नागार्जुन नगरपालिका",
  email: "info@nagarjunmun.gov.np",
  phone: "०१-४६७१७१६,४६७१७१७​",
  address1: "नागार्जुन नगरपालिका",
  address2: "प्रदेश नं. ३, नेपाल",
  letterheadTitle: "नागार्जुन नगरपालिका",
  letterheadType: "नगर कार्यपालिकाको कार्यलय",
  letterheadAddress1: "हरिसिद्धि, सीतापाईला, काठमाडौँ",
  letterheadAddress2: "बागमती प्रदेश, नेपाल",
  district: "काठमाण्डौं",
  // baseUrl: "https://naksapass-nagarjun-api.addon.com.np",
  baseUrl: "http://192.168.1.80:5000",
  // baseUrl: "http://localhost:5000",

  myDomain: "https://naksapass.nagarjunmun.gov.np",
};
var munName = "Naksa Pass | Nagarjun Municipality";
window.document.title = munName;
