...
# do
    ...
    # sudo docker exec addondb /usr/bin/mysqldump -u root --password=$PASSWORD digitization_panauti_db > $PANAUTIDIR/$(date +%Y%m%d)digi_panauti.sql
    # # echo "done dumping digipanauti"

    # # echo "starting dump of villanepal rms_db..."
    # sudo docker exec addondb /usr/bin/mysqldump -u root --password=$PASSWORD rms_db > $VILLANEPALDIR/$(date +%Y%m%d)villanepal.sql
    # # echo "done dumping villanepal rms_db"

    # # echo "starting dump of samadristi samadristi_db..."
    # sudo docker exec addondb /usr/bin/mysqldump -u root --password=$PASSWORD samadristi_db > $SAMADRISTIDIR/$(date +%Y%m%d)samadristi.sql
    # # echo "done dumping samadristi samadristi_db"

    # # echo "starting dump of bpstest bpstest_db..."
    # sudo docker exec addondb /usr/bin/mysqldump -u root --password=$PASSWORD bpstest_db > $BPSTESTDIR/$(date +%Y%m%d)bpstest.sql
    # # echo "done dumping bpstest bpstest_db"

# echo "starting dump of bpstest alldbs..."
    sudo docker exec addondb /usr/bin/mysqldump -u root --password=$PASSWORD --all-databases > $ALLDBSDIR/$(date +%Y%m%d)all_dbs.sql
...
# sudo aws s3 sync /home/ubuntu/mysqlbackups s3://aws-db-server/
