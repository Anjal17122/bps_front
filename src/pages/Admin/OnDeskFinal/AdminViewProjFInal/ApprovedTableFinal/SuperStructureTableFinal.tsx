...
const SuperStructureTableFinal = ({ onViewProject }: Props) => {
  const { SuperStructurePage } = useStorePage();
  const { disabled } = useStoreViewProj();
  const { SuperStructureUrl } = useStoreUrl();
  ...
  const { isLoading, data: approved } = useQuery<
    ...
    { message: string }
  >({
    queryKey: [MyQueries.SuperStructure, SuperStructureUrl, SuperStructurePage],
    queryFn: () =>
      GETonDeskAll(
        SuperStructureUrl + `${SuperStructurePage}&size=${size}`,
        messageApi
      ).then((res) => res),
  ...
  return (
    <div className="MyTableOuter">
      ...
      <table className="MyTable">
        <thead>
          <tr>
            ...
            <th>Date</th>
            {/* <th style={{ width: "160px" }}>Report Submit Status</th> */}
            <th style={{ width: 110 }}>
              Certificate &nbsp;
          ...
          {approved?.data.map((project) => (
            <tr key={project?.id}>
              ...
              <td>
                ...
                {/*  <>*/}
                {/*    <Link*/}
                {/*      to={`/noticepdf/${project.id}/${project.chalaniNo}/${project.patraSankhya}/${project.noticePublishedAt}/${project.creationDate}`}*/}
                {/*      target="_blank"*/}
              ...
              <td>
                <More project={project} type="Approved" />
              </td>
            </tr>
          ))}
        </tbody>
      ...
      <Pagination
        current={SuperStructurePage + 1}
        disabled={disabled}
        onChange={(page) =>
          dispatchPage({ type: AcP.setSuperStructurePage, payload: page - 1 })
        }
...
export default SuperStructureTableFinal;
