...
const ApprovedTableFinal = ({ onViewProject }: Props) => {
  const { ApprovedPage } = useStorePage();
  const { disabled } = useStoreViewProj();
  const { ApprovedUrl } = useStoreUrl();
  ...
  const { isLoading, data: approved } = useQuery<
    ...
    { message: string }
  >({
    queryKey: [MyQueries.Approved, ApprovedUrl, ApprovedPage],
    queryFn: () =>
      GETonDeskAll(
        ApprovedUrl + `${ApprovedPage}&size=${size}`,
        messageApi
      ).then((res) => res),
  ...
  return (
    <div className="MyTableOuter">
      ...
      <Pagination
        ...
        disabled={disabled}
        onChange={(page) =>
          dispatchPage({ type: AcP.setApprovedPage, payload: page - 1 })
        }
...
export default ApprovedTableFinal;
