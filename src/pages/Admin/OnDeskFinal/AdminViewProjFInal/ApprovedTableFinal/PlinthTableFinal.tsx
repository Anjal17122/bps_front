...
const PlinthTableFinal = ({ onViewProject }: Props) => {
  const { PlinthPage } = useStorePage();
  const { disabled } = useStoreViewProj();
  const { PlinthUrl } = useStoreUrl();
  const [messageApi, contextHolder] = message.useMessage();

  const queryClient = useQueryClient();

  const { isLoading, data: plinth } = useQuery<ResOnDesk, { message: string }>({
    queryKey: [MyQueries.Plinth, PlinthUrl, PlinthPage],
    queryFn: () =>
      GETonDeskAll(PlinthUrl + `${PlinthPage}&size=${size}`, messageApi).then(
        (res) => res
      ),
  ...
  return (
    <div className="MyTableOuter">
      ...
      <Pagination
        current={PlinthPage + 1}
        disabled={disabled}
        onChange={(page) =>
          dispatchPage({ type: AcP.setPlinthPage, payload: page - 1 })
        }
        total={plinth?.total ?? 100}
...
export default PlinthTableFinal;
