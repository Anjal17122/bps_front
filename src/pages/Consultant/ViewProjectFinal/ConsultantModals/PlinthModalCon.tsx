...
const PlinthModalCon = () => {
  ...
  useEffect(() => {
    GETPlinthDatas(currentPid, messageApi).then((res) => {
      ...
      localStorage.setItem("nirman", JSON.stringify(res.data.nirmanSampanna));
      ...
      <Row className="FirstRow">
        ...
        <Col span="8" className="ColFirst reportRows">
          {!nirmanSampannaSubmitted ? (
            <Tooltip title="Fill Building Completion Report">
              <Link to={`/user/buildingreport/save/${currentPid}`}>
                <Button
                  icon={<UploadOutlined style={{ color: "white" }} />}
                  type="primary"
                />
        ...
        <Col span="6" className="reportRows">
          {certificateData?.nirmanSampanna?.status === "VERIFIED"
            ? approved
            : certificateData?.nirmanSampanna?.status === "UNVERIFIED"
            ? unapproved
            : correction}
        </Col>
      </Row>
