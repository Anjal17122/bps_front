...
interface Props {
  onSubmit: (val: NBC105typ) => void;
  data: NBC105typ;
  edit: boolean;
}

const NBC105edit = ({ onSubmit, data, edit }: Props) => {
  const [adoptedCode, setAdoptedCode] = useState<string>(""); // State to track adopted code

  const handleAdoptedCodeChange = (value: any) => {
    setAdoptedCode(value[0]); // Assuming the value is in the first element
  };
  useEffect(() => {
    if (data.ACSeisDes && data.ACSeisDes == "NBC 105") {
      setAdoptedCode("NBC 105");
    }
  }, []);
  return (
