...
export default function FormListOldPermitLandOwners({
  name,
}: {
  name: string;
}) {
            ...
            {fields.map(({ key, name, ...restField }) => (
              ...
              >
                ...
                <Col {...ColHeight(8)}>
                  <Form.Item
                    {...restField}
                    label={IsOrg ? ft.orgNameEng : ft.clientNameEng}
                    name={[name, "name"]}
                    rules={[{ required: true }]}
                  >
                    <Input
                      placeholder={IsOrg ? ft.orgNameEng : ft.clientNameEng}
                    />
                  </Form.Item>
                </Col>
                <Col {...ColHeight(8)}>
                  <Form.Item
                    {...restField}
                    label={IsOrg ? ft.orgNepali : ft.clientNameNep}
                    name={[name, "nameNep"]}
                    rules={[{ required: true }]}
                  >
                    <Input
                      placeholder={IsOrg ? ft.orgNepali : ft.clientNameNep}
                    />
                  </Form.Item>
                </Col>
                <Col {...ColHeight(8)}>
                  <Form.Item
                    {...restField}
                    label={IsOrg ? ft.orgRegNo : ft.clientCitizenshipNumber}
                    name={[name, "citizenshipNo"]}
                    rules={[{ required: true }]}
                  >
                    <Input
                      placeholder={
                        IsOrg ? ft.orgRegNo : ft.clientCitizenshipNumber
                      }
                    />
                  </Form.Item>
                </Col>
